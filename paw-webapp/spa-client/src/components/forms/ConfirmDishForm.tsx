...
const ConfirmDishForm: FC<Props> = ({
  ...
  const { reservation, reloadItems, updateReservation, restaurant, discount } = useContext(ReservationContext);
      ...
      <Dialog open={isOpen}>
        ...
        <DialogContent>
          ...
          <Grid container marginY={3}>
            ...
            <Typography style={{ textDecoration }} variant="caption" marginRight={discount ? 1 : 0}>
              ${dish.price}
            </Typography>
            {discount && restaurant && <Typography variant="caption" color="blue">{(1 - restaurant?.discountCoefficient) * dish.price}</Typography>}
            <Grid item container xs={12} justifyContent="space-between">
              <Grid item sx={{ display: "flex", alignItems: "center" }} xs={8} >
                <Typography>{t('forms.confirmDish.subtotal')}</Typography>
                <Typography style={{ textDecoration }} >{ dish.price * qty }</Typography>
                {discount && restaurant && <Typography variant="caption" color="blue">{(1 - restaurant?.discountCoefficient) * dish.price * qty}</Typography>}
              </Grid>
